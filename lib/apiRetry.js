...
function apiRetry( method, url, headers, data, timoutinseconds = 600, retries = 3, delayAfterRequestInSec = 5 ) {
  return new Promise(async (resolve, reject) => {
    for (let i = 1; i <= retries; i++) {
      try {
        x = await apicall(method, url, headers, data, timoutinseconds);
        resolve(x);
      } catch (error) {
